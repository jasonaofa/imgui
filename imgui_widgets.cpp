...
static const char* PatchFormatStringFloatToInt(const char* fmt)
{
    ...
    if (fmt_end > fmt_start && fmt_end[-1] == 'f')
    {
        ...
        if (fmt_start == fmt && fmt_end[0] == 0)
            return "%d";
        ImStrv tmp_format;
        ImFormatStringToTempBuffer(&tmp_format, "%.*s%%d%s", (int)(fmt_start - fmt), fmt, fmt_end); // Honor leading and trailing decorations, but lose alignment/precision.
        return tmp_format.Begin;
#else
